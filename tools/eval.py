...
from sfdm.dataset.facescape_multi_dataset import FaceScape as TrainFacescape
from sfdm.dataset.facescape_eval_dataset import FaceScape as EvalFacescape
import sfdm.utils.general as utils
import sfdm.utils.plots as plt
from glob import glob
from easydict import EasyDict as edict
import yaml
import lpips
from sfdm.utils import fileio, summary
...
def main(args):
    ...
    model = SFDM.load_from_checkpoint(args.ckpt, config=config, train_config=train_config, id2idx=id2idx,
                                              exp2idx=exp2idx, strict=False).to(device).eval() 
    config = model.config
    exp_name = config.expname
    if eval_type == 'trainset':
        test_dataset = TrainFacescape(config, config.dataset.data_dir, 4, split='train', facescape_id=config.dataset.facescape_id, facescape_exp=config.dataset.facescape_exp)
    elif eval_type == 'testset':
        test_dataset = EvalFacescape(config, config.dataset.data_dir, 4, config.get('test_view_num', 20))
    elif eval_type == 'evalset':
        test_dataset = TrainFacescape(config, config.dataset.data_dir, 4, split='test', facescape_id=config.dataset.facescape_id, facescape_exp=config.dataset.facescape_exp)
    ...
    if not config.get('is_stage1', False):
        summary.export_eval(config, model.decoder, model.model, save_path, device, id_latent, exp_latent,
    ...
    for data_index, (indices, model_input, ground_truth) in enumerate(tqdm(test_loader)):
        if config.get('spec_edit', False):
            config['spec_rate'] = 0.5 + data_index / test_dataset.__len__()
        ...
        id = random.randint(0, len(config.dataset.facescape_id) - 1)
        id_latent = model.id_lat_vecs(torch.tensor(id).to(device)).unsqueeze(0)
        exp_latent = model.exp_lat_vecs(torch.tensor(0).to(device)).unsqueeze(0)
        brdf_latent = model.brdf_lat_vecs(torch.tensor(id).to(device)).unsqueeze(0)
        if eval_type == 'trainset':
            calibration_code = model.calibration_lat_vecs(torch.tensor(indices).to(device))
            model_input['calibration_code'] = calibration_code
        else:
            calibration_code = model.calibration_lat_vecs(torch.tensor([1]).to(device))
            model_input['calibration_code'] = calibration_code
        L = model.sh_light(torch.tensor([id]).to(calibration_code.device))
        L = L.reshape(-1, 121, 3)
        ...
        if eval_type == 'trainset' or eval_type == 'evalset':
            ...
            img = Image.fromarray(rgb_eval)
            print('fit_path', os.path.join(save_path, '{1}_{0}_fit.png'.format('%03d' % out_num, 1)))
            ...
            loss_fn_v0 = lpips.LPIPS(net='alex', version=0.1).eval()
            trans_pred = torch.FloatTensor(rgb_pred).unsqueeze(0).permute(0, 3, 1, 2)
            trans_gt = torch.FloatTensor(rgb_gt).unsqueeze(0).permute(0, 3, 1, 2)
            loss = loss_fn_v0(trans_pred, trans_gt, normalize=True)
            all_lpips.append(loss.detach())
