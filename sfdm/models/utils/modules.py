...
class FCBlock(MetaModule):
    ...
    def forward(self, coords, params=None, return_feat=False, **kwargs):
        if params is not None:
            params = get_subdict(params, 'net')
        if return_feat:
            #pdb.set_trace()
            tmp = self.net[:-1](coords, params=params)
            output = self.net[-1:](tmp, params=params)
            return {'output': output, 'feature': tmp}
        # if return_feat:
        #     output = self.net(coords, params=params)
        #     return {'output': output, 'feature': None}
        output = self.net(coords, params=params)
        return output
