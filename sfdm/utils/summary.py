import torch
import os
from sfdm.utils import geometry
import numpy as np
...
def export_eval(config, test_decoder, test_render, save_path, device, id, exp, epoch, residual):
    test_decoder.eval()
    residual.eval()
    test_render.eval()
    ...
    for i, pnts in enumerate(torch.split(grid_tensor, 100000, dim=0)):
        decoder_output = test_decoder(pnts.unsqueeze(0).to(torch.float32), exp, id)
        # if config.get('g_as_input', False):
        cat_feature = torch.cat([test_render.compress_sdf_feat(decoder_output['geo_feature']), test_render.compress_sdf_feat(decoder_output['brdf_feature']), decoder_output['albedo_grad']], -1)
        # else:
            # cat_feature = torch.cat([test_render.compress_sdf_feat(decoder_output['geo_feature']), test_render.compress_sdf_feat(decoder_output['brdf_feature'])], -1)
        z.append(decoder_output['sdf'].detach().cpu().numpy() +
                 #residual(pnts.to(torch.float32)).detach().cpu().numpy())
                 residual(pnts.to(torch.float32), cat_feature)[:, 0].detach().cpu().numpy())
